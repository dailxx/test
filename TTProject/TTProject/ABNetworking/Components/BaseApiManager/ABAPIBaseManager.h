...
/*************************************************************************************************/
/*                               ABAPIManagerApiCallBackDelegate                                 */
/*************************************************************************************************/

//api回调
@protocol ABAPIManagerCallBackDelegate <NSObject>
@required
...
/*************************************************************************************************/
/*                               ABAPIManagerCallbackDataReformer                                */
/*************************************************************************************************/
//负责重新组装API数据的对象
/*
 重组数据的场景：
 ...
 */
@protocol ABAPIManagerDataReformer <NSObject>
@required
/*
...
代码样例：
- (id)manager:(ABAPIBaseManager *)manager reformData:(NSDictionary *)data
{
...
/*
 ...
 3.有一种情况是manager调用api时使用的参数不一定是明文传递的，有可能是从某个变量或者跨越了好多层的对象中来获得参数，那么在调用api的最后一关会有一个参数验证，当参数不对时不访问api，同时自身的errorType将会变为ABAPIManagerErrorTypeParamsError。这个机制可以优化我们的app。
 ...
 */
@protocol ABAPIManagerValidator <NSObject>
@required
/*
...
/*************************************************************************************************/
/*                                ABAPIManagerParamSourceDelegate                                */
/*************************************************************************************************/
//让manager能够获取调用API所需要的数据
@protocol ABAPIManagerParamSource <NSObject>
@required
 ...
 */
typedef NS_ENUM (NSUInteger, ABAPIManagerErrorType){
    ABAPIManagerErrorTypeDefault,       //没有产生过API请求，这个是manager的默认状态。
    ABAPIManagerErrorTypeSuccess,       //API请求成功且返回数据正确，此时manager的数据是可以直接拿来使用的。
    ABAPIManagerErrorTypeNoContent,     //API请求成功但返回数据不正确。如果回调数据验证函数返回值为NO，manager的状态就会是这个。
    ABAPIManagerErrorTypeParamsError,   //参数错误，此时manager不会调用API，因为参数验证是在调用API之前做的。
    ABAPIManagerErrorTypeTimeout,       //请求超时。CTAPIProxy设置的是20秒超时，具体超时时间的设置请自己去看CTAPIProxy的相关代码。
    ABAPIManagerErrorTypeNoNetWork      //网络不通。在调用API之前会判断一下当前网络是否通畅，这个也是在调用API之前验证的，和上面超时的状态是有区别的。
};

typedef NS_ENUM (NSUInteger, ABAPIManagerRequestType){
    ABAPIManagerRequestTypeGet,
    ABAPIManagerRequestTypePost,
    ABAPIManagerRequestTypePut,
    ABAPIManagerRequestTypeDelete
};
...
/*************************************************************************************************/
/*
 ABAPIBaseManager的派生类必须符合这些protocal
 */
@protocol ABAPIManager <NSObject>
...
/*************************************************************************************************/
/*                                    ABAPIManagerInterceptor                                    */
/*************************************************************************************************/
/*
 ABAPIBaseManager的派生类必须符合这些protocal
 */
@protocol ABAPIManagerInterceptor <NSObject>

@optional
- (BOOL)manager:(ABAPIBaseManager *)manager beforePerformSuccessWithResponse:(ABURLResponse *)response;
- (void)manager:(ABAPIBaseManager *)manager afterPerformSuccessWithResponse:(ABURLResponse *)response;

- (BOOL)manager:(ABAPIBaseManager *)manager beforePerformFailWithResponse:(ABURLResponse *)response;
- (void)manager:(ABAPIBaseManager *)manager afterPerformFailWithResponse:(ABURLResponse *)response;

- (BOOL)manager:(ABAPIBaseManager *)manager shouldCallAPIWithParams:(NSDictionary *)params;
- (void)manager:(ABAPIBaseManager *)manager afterCallingAPIWithParams:(NSDictionary *)params;
...
/*                                       CTAPIBaseManager                                        */
/*************************************************************************************************/
@interface ABAPIBaseManager : NSObject

@property (nonatomic, weak) id<ABAPIManagerCallBackDelegate> delegate;
@property (nonatomic, weak) id<ABAPIManagerParamSource> paramSource;
@property (nonatomic, weak) id<ABAPIManagerValidator> validator;
// 里面会调用到NSObject的方法，所以这里不用id
@property (nonatomic, weak) NSObject<ABAPIManager> *child;
@property (nonatomic, weak) id<ABAPIManagerInterceptor> interceptor;

/*
 * baseManager是不会去设置errorMessage的，派生的子类manager可能需要给controller提供错误
 * 信息。所以为了统一外部调用的入口，设置了这个变量。派生的子类需要通过extension来在保证
 * errorMessage在对外只读的情况下使派生的manager子类对errorMessage具有写权限。
 */
@property (nonatomic, copy, readonly) NSString *errorMessage;
@property (nonatomic, readonly) ABAPIManagerErrorType errorType;
@property (nonatomic, strong) ABURLResponse *response;
